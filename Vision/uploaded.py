...
class Tester:
    def __init__(self, config_parser):
        ...
        parser.parse()
        #commented out labels that Axon provides
        ...
        while True:
            ...
            for i in range(len(boxes)):
                if scores[i] > 0.5:
                    ...
                    red = [40,  40, 205]
                    redtolerance = [35, 35, 50]
                    blue = [180, 90, 50]
                    bluetolerance = [70, 40, 40]

                    cropped = frame_cv2[ymin:ymax, xmin: xmax]
                    #May come back and change to hsv using this code
                    #Would have to change values above ^
                    #hsvcropped = cv2.cvtColor(cropped, cv2.COLOR_BGR2HSV)
